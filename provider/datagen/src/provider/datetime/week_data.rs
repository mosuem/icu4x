    ...
    fn supported_locales(&self) -> Result<Vec<DataLocale>, DataError> {
        ...
        let regions: HashSet<DataLocale> = week_data
            ...
            .filter_map(|t| match t {
                &cldr_serde::week_data::DEFAULT_TERRITORY => Some(None),
                cldr_serde::week_data::Territory::Region(r) => Some(Some(*r)),
                _ => None,
            })
...
impl DataProvider<WeekDataV1Marker> for crate::DatagenProvider {
    fn load(&self, req: DataRequest) -> Result<DataResponse<WeekDataV1Marker>, DataError> {
        ...
        let territory = req
            .locale
            .region()
            .map(|v| -> Result<cldr_serde::week_data::Territory, DataError> { Ok(cldr_serde::week_data::Territory::Region(v)) })
            .transpose()?
            .unwrap_or_else(|| cldr_serde::week_data::DEFAULT_TERRITORY.clone());
        ...
        Ok(DataResponse {
            ...
            payload: Some(DataPayload::from_owned(WeekDataV1 {
                first_weekday: week_data
                    .first_day
                    .get(&territory)
                    .or_else(|| week_data.first_day.get(&cldr_serde::week_data::DEFAULT_TERRITORY))
                ...
                min_week_days: week_data
                    .min_days
                    .get(&territory)
                    .or_else(|| week_data.min_days.get(&cldr_serde::week_data::DEFAULT_TERRITORY))
                    .ok_or(DataError::custom(
