...
fn get_era_code_map(calendar: &str) -> BTreeMap<String, TinyStr16> {
    match calendar {
        ...
        "japanese" | "japanext" => crate::provider::calendar::japanese::get_era_code_map(),
        "coptic" => [
            // Before Diocletian
...
macro_rules! symbols_from {
    ...
    ([$symbols: path, $name2: ident], $ctx:ty) => {
        ...
        impl cldr_serde::ca::Contexts<$symbols> {
        ...
        impl cldr_serde::ca::StandAloneWidths<$symbols> {
            // Helper function which returns `None` if the two groups of symbols overlap.
            pub fn get_unaliased(&self, other: &cldr_serde::ca::FormatWidths<$symbols>) -> Option<Self> {
        ...
        impl cldr_serde::ca::FormatWidths<$symbols> {
